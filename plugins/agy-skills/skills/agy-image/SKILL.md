...
python3 ${CLAUDE_SKILL_DIR}/scripts/agy_image.py "<英語の画像プロンプト>" \
  ...
  [--transparent black|white|chroma] [--trim]
```
...
1. 用途と保存先を決める(プロジェクト内の `images/` や `assets/` など)。スライドの任意の場所に載せる部品なら最初から `--transparent` を選ぶ(背景込みで作ると後から抜けない)。ファイル名は内容が分かる英小文字にすると `--name` を自動導出できる。
...
agy(generate_image)はアルファ付き画像を返せない(常に JPEG)。そこでスクリプトは `--transparent` を付けると、単色背景で生成するようプロンプトに追記し、手元で背景を抜いて透過 PNG にする。追加ライブラリは不要(Pillow があれば使い、無ければ macOS の sips と標準ライブラリで処理)。
...
- `--trim` で不透明部分の外接矩形(+8px)に切り抜く。スライドに置くときの位置合わせが楽になる。
